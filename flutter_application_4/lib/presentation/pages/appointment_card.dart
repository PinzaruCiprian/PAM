...
import 'package:flutter_application_4/domain/entities/appointment_entity.dart';
    ...
    return BlocBuilder<DoctorDetailsBloc, DoctorDetailsState>(
      builder: (context, state) {
        if (state is DoctorDetailsLoaded) {
          final AppointmentEntity appointment = state.details.appointment;
          return Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                buildHeader({'type': appointment.type, 'currency': appointment.currency, 'fee': appointment.fee}),
                buildHospitalInfo({'name': appointment.hospital.name, 'location': appointment.hospital.location, 'wait_time': appointment.hospital.waitTime, 'more_clinics': appointment.hospital.moreClinics}),
                const SizedBox(height: 12),
                buildDivider(),
                const SizedBox(height: 12),
                buildDateAndTimeSection({'available_days': appointment.availableDays.map((d) => {'day': d.day, 'slots': d.slots}).toList()}),
              ],
            ),
          );
        }
